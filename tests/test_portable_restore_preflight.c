...
static void test_payload_inventory_parallel_workers(void)
{
    printf(BLUE "::" NC " payload inventory overlaps regular-file metadata I/O\n");
    enum { FILE_COUNT = 32 };
    ManifestRoot root = root_for("ROOT", "ROOT", "restored");
    Fixture fixture;
    int opened = fixture_open(&fixture, "payload-parallel", &root, 1);
    check(opened == 0, "parallel payload fixture is created");
    if (opened != 0)
        return;
    make_root_payload(&fixture);

    SidecarEntry entries[FILE_COUNT + 1U];
    char names[FILE_COUNT][32];
    entries[0] = entry_for("ROOT", "", "", SIDECAR_KIND_DIRECTORY, 0);
    for (size_t index = 0; index < FILE_COUNT; index++)
    {
        snprintf(names[index], sizeof(names[index]), "file-%02zu", index);
        char payload[64];
        snprintf(payload, sizeof(payload), "ROOT/%s", names[index]);
        write_file_at(fixture.data_fd, payload, "x");
        entries[index + 1U] = entry_for(
            "ROOT", names[index], names[index], SIDECAR_KIND_REGULAR, 1);
    }
    check(write_sidecar(&fixture, entries, FILE_COUNT + 1U) == 0,
          "parallel payload sidecar is committed");

    portable_restore_preflight_test_configure_payload_pool(0, 0, NULL, NULL);
    PortableRestorePreflightReport serial_report;
    int serial_result = run_preflight(&fixture, &serial_report);
    size_t serial_live_count = serial_report.live_count;
    size_t serial_mapped_root_count = serial_report.mapped_root_count;
    size_t serial_violation_count = serial_report.violation_count;
    off_t serial_estimated_bytes = serial_report.estimated_bytes;
    size_t serial_profile_count = serial_report.profiles.count;
    size_t serial_affected_objects = serial_report.profiles.affected_objects;
    size_t serial_security_xattr_entries =
        serial_report.profiles.security_xattr_entry_count;
    portable_restore_preflight_report_free(&serial_report);

    portable_restore_preflight_test_configure_payload_pool(4, 10, NULL, NULL);
    PortableRestorePreflightReport parallel_report;
    int parallel_result = run_preflight(&fixture, &parallel_report);
    size_t peak_workers =
        portable_restore_preflight_test_payload_peak_workers();
    size_t checked_count =
        portable_restore_preflight_test_payload_checked_count();
    check(serial_result == 0 && parallel_result == 0 &&
              parallel_report.live_count == serial_live_count &&
              parallel_report.mapped_root_count == serial_mapped_root_count &&
              parallel_report.violation_count == serial_violation_count &&
              parallel_report.estimated_bytes == serial_estimated_bytes &&
              parallel_report.profiles.count == serial_profile_count &&
              parallel_report.profiles.affected_objects ==
                  serial_affected_objects &&
              parallel_report.profiles.security_xattr_entry_count ==
                  serial_security_xattr_entries,
          "parallel payload scan preserves the serial preflight report");
    check(peak_workers >= 2,
          "parallel payload scan runs more than one metadata worker concurrently");
    check(checked_count == FILE_COUNT + 1U,
          "parallel payload scan accounts for every root and file before completion");
    portable_restore_preflight_report_free(&parallel_report);

    portable_restore_preflight_test_configure_payload_pool(
        4, 30, names[FILE_COUNT - 1U], NULL);
    PortableRestorePreflightReport delayed_report;
    int delayed_result = run_preflight(&fixture, &delayed_report);
    checked_count = portable_restore_preflight_test_payload_checked_count();
    check(delayed_result == 0 && delayed_report.violation_count == 0 &&
              checked_count == FILE_COUNT + 1U,
          "payload missing-entry sweep waits for a deliberately delayed worker");
    portable_restore_preflight_report_free(&delayed_report);

    portable_restore_preflight_test_reset_payload_pool();
    fixture_close(&fixture);
}

static void test_payload_inventory_parallel_failure(void)
{
    printf(BLUE "::" NC " parallel payload failures remain fail-closed\n");
    enum { FILE_COUNT = 12, BAD_INDEX = 7 };
    ManifestRoot root = root_for("ROOT", "ROOT", "restored");
    Fixture fixture;
    int opened = fixture_open(&fixture, "payload-parallel-failure", &root, 1);
    check(opened == 0, "parallel failure fixture is created");
    if (opened != 0)
        return;
    make_root_payload(&fixture);
    if (mkdirat(fixture.data_fd, "ROOT/deep", 0700) != 0 ||
        mkdirat(fixture.data_fd, "ROOT/deep/branch", 0700) != 0)
        fatal("could not create parallel failure payload directories");

    SidecarEntry entries[FILE_COUNT + 3U];
    char logical[FILE_COUNT][64];
    char leaf[FILE_COUNT][32];
    entries[0] = entry_for("ROOT", "", "", SIDECAR_KIND_DIRECTORY, 0);
    entries[1] = entry_for("ROOT", "deep", "deep",
                           SIDECAR_KIND_DIRECTORY, 0);
    entries[2] = entry_for("ROOT", "deep/branch", "branch",
                           SIDECAR_KIND_DIRECTORY, 0);
    for (size_t index = 0; index < FILE_COUNT; index++)
    {
        snprintf(leaf[index], sizeof(leaf[index]), "file-%02zu", index);
        snprintf(logical[index], sizeof(logical[index]),
                 "deep/branch/%s", leaf[index]);
        char payload[96];
        snprintf(payload, sizeof(payload), "ROOT/%s", logical[index]);
        write_file_at(fixture.data_fd, payload, "x");
        entries[index + 3U] = entry_for(
            "ROOT", logical[index], leaf[index], SIDECAR_KIND_REGULAR,
            index == BAD_INDEX ? 2U : 1U);
    }
    check(write_sidecar(&fixture, entries, FILE_COUNT + 3U) == 0,
          "parallel failure sidecar is committed");

    portable_restore_preflight_test_configure_payload_pool(4, 5, NULL, NULL);
    PortableRestorePreflightReport report;
    int result = run_preflight(&fixture, &report);
    size_t peak_workers =
        portable_restore_preflight_test_payload_peak_workers();
    check(result != 0 && report.violation_count == 1 &&
          ...
          "a deep bad entry still aborts the parallel payload scan exactly once");
    check(peak_workers >= 2,
          "the deep refusal is exercised with genuinely concurrent workers");
    portable_restore_preflight_report_free(&report);
    portable_restore_preflight_test_reset_payload_pool();
    fixture_close(&fixture);
}

static void test_payload_inventory_parallel_duplicate_address(void)
{
    printf(BLUE "::" NC " concurrent payload results cannot claim one address twice\n");
    enum { FILE_COUNT = 12, RUNS = 8 };
    ManifestRoot root = root_for("ROOT", "ROOT", "restored");
    Fixture fixture;
    int opened = fixture_open(&fixture, "payload-parallel-duplicate", &root, 1);
    check(opened == 0, "parallel duplicate-address fixture is created");
    if (opened != 0)
        return;
    make_root_payload(&fixture);
    ...
    check(write_sidecar(&fixture, entries, FILE_COUNT + 1U) == 0,
          "parallel duplicate-address sidecar is committed");
    ...
    for (size_t run = 0; run < RUNS; run++)
    {
        PortableRestorePreflightReport report;
        int result = run_preflight(&fixture, &report);
        if (result == 0 || report.violation_count != 1 ||
            report.root_count != 1 || report.roots == NULL ||
            report.roots[0].violation_count != 1)
            all_refused = 0;
        if (portable_restore_preflight_test_payload_peak_workers() < 2)
            concurrent = 0;
        portable_restore_preflight_report_free(&report);
    }
    check(all_refused,
          "duplicate concurrent results are repeatedly rejected by single-writer address accounting");
    check(concurrent,
          "duplicate-address stress runs actually overlap worker execution");
...
int main(void)
{
    ...
    test_payload_inventory_parallel_workers();
    test_payload_inventory_parallel_failure();
    test_payload_inventory_parallel_duplicate_address();
