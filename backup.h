...
typedef enum {
    ...
    BACKUP_PATHS          /**< Only the caller-supplied paths; no dotfiles, packages, or manifest. */
} BackupMode;
 ...
 * Music, and Projects. BACKUP_PATHS copies only the caller-supplied paths.
 ...
 * @param mode   Selects which files are included (BACKUP_CRITICAL, BACKUP_COMPREHENSIVE, or BACKUP_PATHS).
 * @param paths  NULL-terminated array of absolute paths; required when mode is BACKUP_PATHS, ignored otherwise.
